...
function getComponentImports(exportDefault = true) {
   ...
   return covertToObject(entries.map(entry => {
      const dirOptions = directories.find(dir => minimatch(entry.path, dir.pattern)) || { omit: '' }
...
console.log("🚀 ~ export  configAutoImport.getComponentImports:", flowbiteImports)
